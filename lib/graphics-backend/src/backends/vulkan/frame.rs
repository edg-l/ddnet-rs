...
use super::{render_pass::CanvasSetup, vulkan_types::RenderPassType};
...
#[derive(Debug, Hiarc)]
pub struct FrameRenderOffscreenCanvas {
    pub canvas: FrameRenderCanvas,
    pub setup: Arc<CanvasSetup>,
}

impl FrameRenderOffscreenCanvas {
    fn new(passes_pool: &Pool<Vec<FrameRenderPass>>, setup: Arc<CanvasSetup>) -> Self {
        Self {
            canvas: FrameRenderCanvas::new(passes_pool),
            setup,
        }
    }
}

#[derive(Debug, Hiarc, Default, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
...
pub struct FrameRenderScope {
    ...
    pub offscreen_canvases: FxLinkedHashMap<OffscreenCanvasId, FrameRenderOffscreenCanvas>,
}
    ...
    pub fn canvas_mode_mut(&mut self, index: FrameCanvasIndex) -> &mut FrameRenderCanvas {
        match index {
            ...
            FrameCanvasIndex::Offscreen(id) => {
                &mut self.offscreen_canvases.get_mut(&id).unwrap().canvas
            }
        }
    }
}
...
/// a frame of commands
/// the frame is reset only when swap is called
#[derive(Debug, Hiarc)]
pub struct Frame {
...
impl Frame {
    ...
    pub fn new_offscreen(&mut self, id: OffscreenCanvasId, setup: Arc<CanvasSetup>) {
        self.render.offscreen_canvases.insert(
            id,
            FrameRenderOffscreenCanvas::new(&self.passes_pool, setup),
        );
    }
}
