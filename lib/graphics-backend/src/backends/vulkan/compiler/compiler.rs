...
pub struct ShaderCompiler {
    ...
    pub(crate) cache: Arc<Cache<20231227>>,

    pub(crate) shader_files: HashMap<String, Vec<u32>>,
}

impl ShaderCompiler {
    pub async fn new(ty: ShaderCompilerType, fs: Arc<dyn FileSystemInterface>) -> Self {
        Self {
            ty,
            cache: Arc::new(Cache::new_async("wgsl", &fs).await),
            fs,
