...
impl Benchmark {
    pub fn new(_do_bench: bool) -> Self {
        #[cfg(target_arch = "wasm32")]
        let _do_bench = false;
        let (start_time, cur_diff) = if _do_bench {
            (Some(std::time::Instant::now()), AtomicU64::new(0))
        } else {
            (None, AtomicU64::new(0))
        };

        let offset = if _do_bench {
            let _ = *START_TIME;
    ...
    pub fn bench_multi(&self, name: &str) -> u64 {
        if self.is_active {
            ...
            let diff = self.start_time.unwrap().elapsed().as_nanos() as u64 - cur_diff;
            let tabs: String = (0..self.offset)
                .map(|_| "  ")
                .collect::<Vec<&str>>()
                .join("");
            println!(
                "{}{} took {:.2}s / {:.2}ms / {:.2}us / {:.2}ns / {:.2}ms global",
                tabs,
                name,
                diff as f64 / 1000000000.0,
                diff as f64 / 1000000.0,
                diff as f64 / 1000.0,
                diff,
